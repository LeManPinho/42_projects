...
int		tests_i(t_tout *tout)
{
	...
	else if ((ft_strcmp(tout->lines[ft_tablen(tout->lines) - 1], "cd") == 0) || (ft_strcmp(tout->lines[ft_tablen(tout->lines) - 2], "cd") == 0))
	{
		free_tab(tout->envcpy, ft_tablen(tout->envcpy));
		tout->envcpy = env_i_stuff(tout);
		gocd_i(tout);
...
void	gocd_i(t_tout *tout)
{
	...
	if (ft_strcmp(tout->lines[ft_tablen(tout->lines) - 1], "cd") == 0)
	{
		if (ft_getenv(tout->envcpy, "HOME=") == NULL)
			return ;
		tout->lines[ft_tablen(tout->lines) - 1] = ft_strdup(ft_getenv(tout->envcpy, "HOME=") + 5);
	}
